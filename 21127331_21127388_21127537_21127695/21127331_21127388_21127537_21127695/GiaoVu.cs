...
namespace _21127331_21127388_21127537_21127695
{
    public partial class GiaoVu : Form
    {
        ...
        private void dtgdk_masv_TextChanged(object sender, EventArgs e)
        {
            searchdk.Stop();
            searchdk.Start();
        }
        ...
        {
            searchdk.Stop();
            SearchAndFill_DangKy(dtgdk_masv.Text);
        }
        ...
        private void dtgdk_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                ...
                txt_dk_masv.Text = row.Cells["MASV"].Value.ToString();
        ...
        private void SearchAndFill_DangKy(string masv)
        {
            ...
            try
            {
                string query = $"select * from OLS_ADMIN.uv_GiaoVu_DANGKY where HK={hki}";

                if (!string.IsNullOrEmpty(masv) && masv.Trim() != "")
                {
                    query += " AND MASV = :masv";
                }
                ...
                using (OracleCommand cmd = new OracleCommand(query, conn))
                {
                    ...
                    using (OracleDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        dtgdk.Columns["DIEMTH"].Visible = false;
                        dtgdk.Columns["DIEMQT"].Visible = false;
                        dtgdk.Columns["DIEMCK"].Visible = false;
                        dtgdk.Columns["DIEMTK"].Visible = false;
        ...
        public class DataToPass
        {
            ...
            public string Dk_masv { get; set; }
            public string Dk_magv { get; set; }
            public string Dk_mahp { get; set; }
            public string Dk_hki { get; set; }
            public string Dk_nam { get; set; }
            public string Dk_mact { get; set; }
        }
        ...
        {
            ThemDangKy_GiaoVu themdkgv = new ThemDangKy_GiaoVu();
            DataToPass data = new DataToPass();
            data.Masv = txt_masv.Text;
            data.Mact = txt_mact_sv.Text;

            themdkgv.SetData(data);
            themdkgv.ShowDialog();
        }
